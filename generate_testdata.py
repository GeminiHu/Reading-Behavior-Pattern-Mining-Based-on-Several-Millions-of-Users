...
for k in xrange(1,32):
    ...
    #存储下每个文件需要记录的数据
    #存储每日pv数据
    with open('F://WSC225//ReadingOnline//test//pv//pv'+b+'.pickle', 'wb') as f_pickle:
        pickle.dump(pv[k-1], f_pickle)
    f_pickle.close()
    #存储每日c3数据
    with open('F://WSC225//ReadingOnline//test//city//city_date'+b+'.pickle', 'wb') as f_pickle:
...
#存储用户访问数据
with open('F://WSC225//ReadingOnline//test//interval.pickle', 'wb') as f_pickle:
